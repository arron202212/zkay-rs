...
use crate::types::{Value,
    AddressValue, BlockStruct, MsgStruct, PublicKeyValue, TxStruct,
};
...
impl<P:ZkayProverInterface,W> Web3BlockchainBase<P,W>{
    pub fn new(prover:P)->Self{
        Self{prover,_lib_addresses:BTreeMap::new(),_pki_contract:BTreeMap::new(),_web3:PhantomData}
    }
}
impl<P:ZkayProverInterface,W> ZkayBlockchainInterface<P> for Web3BlockchainBase<P,W> {
 fn lib_addresses_mut(&mut self) -> &mut BTreeMap<String, JsonValue> { &mut self._lib_addresses }
   fn prover(&self) -> &P { &self.prover }
   ...
   fn deploy_solidity_contract<T: Clone + Default, V: Clone + Default>(
    ...
    fn _req_public_key(
        ...
        Ok(Value::<u8,PublicKeyValue>::new(
            self._req_state_var::<String>(
                &pki_contract,
                "getPk",
                address,
            ).into_bytes(),
            Some(crypto_params.clone()),None
        ))
    }

    fn _announce_public_key(&mut self,    address: &str,
        pk: &String,
        crypto_params: &CryptoParams,
    ) {
            ...
            with_context_block!(var _a= log_context("constructor")=>{
                with_context_block!(var _b=log_context(&format!("{verifier_name}"))=>{
                    ...
                    with_context_block!(var _tm= time_measure("transaction_full",false,false)=>{
                        vf.insert(verifier_name.clone(),self._deploy_contract(sender, cout,&String::default(),None).unwrap());
                    });
            });
            });
        }
    ...
    fn _connect_libraries(&mut self) {
        ...
        // let mut _pki_contract=BTreeMap::new();
            ...
            self._pki_contract.insert(crypto_params,self._verify_contract_integrity(
...
//  *self._pki_contract.borrow_mut()=_pki_contract;
    ...
    fn _connect(&self, project_dir: &str, contract: &str,  address: JsonValue
    ) -> eyre::Result<JsonValue> {
        let filename = PathBuf::from(project_dir).join("contract.sol");
        let cout = self.compile_contract(&filename, contract,None,".");
    ...
    fn _verify_library_integrity(
        ...
        let cname = get_contract_names(&sol_with_libs_filename.to_string_lossy().to_string())[0].clone();
        let actual_code = self.__normalized_hex("self.w3.eth.getCode(contract_with_libs_addr)".to_owned());
        assert!(
        ...
        let code_with_placeholders = self
            .__normalized_hex(self.compile_contract(sol_with_libs_filename, &cname,None,".")["deployed_bin"].to_string());
        ...
        for (lib_name, lib_sol) in libraries {
            ...
            if let Some(lib_address_offset) = lib_address_offset {
                ...
                self._verify_contract_integrity(&lib_address, &lib_sol, None,Some(lib_name.clone()), true,None);
    ...
    fn _verify_zkay_contract_integrity(
        ...
        let sol_file = self.__hardcoded_external_contracts_ctx(project_dir, &pki_verifier_addresses);
        self._verify_contract_integrity(address, &PathBuf::from(sol_file), None,None,false,Some(project_dir.to_owned()));
    }
...
impl<P:ZkayProverInterface,W> Web3BlockchainBase<P,W> {
    ...
    fn _deploy_contract(
        ...
        let tx_receipt = self._transact(&"contract".into(), sender, "constructor", args, wei_amount);
        // let contract = self.w3.eth.contract(
...
impl<P:ZkayProverInterface,Web3TesterBlockchain> Web3BlockchainBase<P,Web3TesterBlockchain>{
    ...
    fn _connect_libraries(&self) {
        ...
        for crypto_params in CFG.lock().unwrap().all_crypto_params() {
            ...
            let pki_contract_code = library_contracts::get_pki_contract(&CryptoParams::new(crypto_params.clone()));
            let pki_contract_name = CFG.lock().unwrap().get_pki_contract_name(&crypto_params);
            let pki_sol = save_to_file(
                Some(tmpdir.clone()),
            ...
            let contract =
                self._deploy_contract(sender, self.compile_contract(&PathBuf::from(pki_sol), &pki_contract_name,None,"."),&String::new(),None);
        ...
        for lib in CFG.lock().unwrap().external_crypto_lib_names() {
            let out = self._deploy_contract(sender, self.compile_contract(&PathBuf::from(verify_sol.clone()), &lib,None,"."),&String::new(),None);
    ...
    fn _gas_heuristics(&self, sender: &str, tx: &str) -> i32 {
        MAX_GAS_LIMIT
    }
}
