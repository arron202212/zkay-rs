...
// use alloy_eips::eip4844::builder::BuildableSidecar;
...
impl<P: Provider<AnyNetwork>> CastTxBuilder<P, InitState> {
    ...
    pub async fn new(provider: P, tx_opts: TransactionOpts, config: &Config) -> Result<Self> {
        ...
        if let Some(fee_token) = tx_opts.tempo.fee_token {
            tx.other.insert(
                "feeToken".to_string(),
                serde_json::to_value(fee_token).unwrap(),
            );
        }
        ...
        if let Some(nonce_key) = tx_opts.tempo.sequence_key {
            tx.other.insert(
                "nonceKey".to_string(),
                serde_json::to_value(nonce_key).unwrap(),
            );
        }
...
impl<P: Provider<AnyNetwork>> CastTxBuilder<P, InputState> {
    ...
    async fn _build(
        ...
        self.tx
            .set_input_kind(self.state.input.clone(), TransactionInputKind::Both);
    ...
    async fn resolve_auth(&mut self, sender: SenderKind<'_>, tx_nonce: u64) -> Result<()> {
        ...
        let address_auth_count = auths
            .iter()
            .filter(|a| matches!(a, CliAuthorizationList::Address(_)))
            .count();
...
async fn decode_execution_revert(data: &RawValue) -> Result<Option<String>> {
    ...
    let Some(selector) = err_data.get(..4) else {
        return Ok(None);
    };
    if let Some(known_error) = SignaturesIdentifier::new(false)?
        .identify_error(selector.try_into().unwrap())
        .await
    {
            ...
            && let Ok(error) = known_error.decode_error(&err_data)
        {
            write!(
                decoded_error,
                "({})",
                format_tokens(&error.body).format(", ")
            )?;
        }
