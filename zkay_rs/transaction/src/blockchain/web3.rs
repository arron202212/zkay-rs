...
impl Web3 {
    pub fn get_balance(&self, who: &str) -> String {
    ...
    pub fn get_block(&self, block: &str) -> String {
    ...
    pub fn eth_accounts(&self) -> String {
        run(Web3Subcommand::Rpc(RpcArgs::parse_from([
            "method",
            "eth_accounts",
        ])))
    }
    pub fn eth_coinbase(&self) -> String {
        run(Web3Subcommand::Rpc(RpcArgs::parse_from([
            "method",
            "eth_coinbase",
        ])))
    }
    pub fn get_code(&self, who: &str) -> String {
        let who = NameOrAddress::from_str(who).unwrap();
    ...
    pub fn wait_for_transaction_receipt(&self, tx_hash: &str) -> String {
    ...
    pub fn to_checksum_address(&self, address: &str) -> String {
        format!("{}", Address::from_str(address).unwrap().to_checksum(None))
    }
    pub fn solidity_keccak(&self, bytes: Vec<u8>) -> String {
...
fn run(cmd: Web3Subcommand) -> String {
    match cmd {
        Web3Subcommand::Balance {
            ...
            let account_addr = utils::block_on(who.resolve(&provider)).unwrap();
            ...
            match erc20 {
                ...
                None => {
                    let value =
                        utils::block_on(Cast::new(&provider).balance(account_addr, block)).unwrap();
                    if ether {
                        format!(
                            "{}",
        ...
        Web3Subcommand::GasPrice { rpc } => {
            ...
            format!(
                "{}",
                utils::block_on(Cast::new(provider).gas_price()).unwrap()
            )
        }
        Web3Subcommand::Rpc(cmd) => utils::block_on(cmd.run()).unwrap(),
        Web3Subcommand::Code {
            block,
            who,
            disassemble,
            ...
            let config = Config::from(&rpc);
            let provider = utils::get_provider(&config).unwrap();
            let who = utils::block_on(who.resolve(&provider)).unwrap();
            format!(
                "{}",
                utils::block_on(Cast::new(provider).code(who, block, disassemble)).unwrap()
            )
        }
...
mod tests {
    ...
    fn test_web3_eth_accounts() {
        ...
        assert_eq!(&web3.eth_accounts(),"[\"0x0f08283db051c5bf85acc4b91a257d87a0c58649\",\"0x85372c241942d72ca13bd510d6e2b71f0f1c2959\",\"0x7b0b1a92ecd80da8f8a1340f24dfefed17dc2dbe\",\"0x195825ecaaddb2c0885d2505da2ba4383ab469fa\",\"0xb59152d3037f5d5363b5e29c6ad2cabe565136a2\",\"0xaf2c727ef24f6d5d61990e9b0e0ae65ad25b886f\",\"0x6b66e3200b1ac53e4c127048dfa0828549c95664\",\"0xbb25fc005ad3cf67064aee9bacef57952eb183c0\",\"0xcb586fc7da30c3799d02e336c1bc36ec9a9a2587\",\"0x3a49a5b08b1d03d253eab323b5e3920bc9f9d4d1\"]");
    }
    #[test]
    ...
    fn test_web3_wait_for_transaction_receipt() {
        ...
        assert!(!web3
            ...
            .is_empty());
        assert_eq!(
            &web3.wait_for_transaction_receipt("0xc619af4165c618a31b879d81562d951cffeb9f6e2d2dd6de313e4db0cbb8899f"),
