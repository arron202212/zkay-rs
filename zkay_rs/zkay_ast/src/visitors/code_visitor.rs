    ...
    ($struct_name: ident) => {
        impl $struct_name {
            ...
            fn handle(&self, e: &ListUnion) -> eyre::Result<<Self as AstVisitor>::Return> {
                match e {
                    ListUnion::String(e) => Ok(e.to_owned()),
            ...
            pub fn visit_IfStatement(
                    ...
                    .else_branch
                {
                    let e = self.visit_single_or_list(
                        SingleOrListUnion::AST(else_branch.clone().into()),
                        "",
                    )?;
                    ret += format!("\n else {e}").as_str();
                }
                    ...
                    .map_or(String::new(), |update| {
                        ...
                        format!(
                            ...
                            self.visit_single_or_list(
                                SingleOrListUnion::AST(update.clone().into()),
                                ""
                            )
                            .unwrap_or_default()
                            .replace(';', "")
            ...
            pub fn visit_Array(
                ...
                let e = expr.as_ref().map_or(String::new(), |_expr| {
                    ...
                    //     _expr.get_ast_type()
                    // );
                    self.visit(_expr).unwrap()
                });
                Ok(format!("{t}[{e}]"))
            }

            pub fn visit_CipherText(
                &self,
                ast: &ASTFlatten,
            ) -> eyre::Result<<Self as AstVisitor>::Return> {
                // println!(
            ...
            pub fn visit_Parameter(
                ...
                let i = ast
                    ...
                    .idf()
                    .as_ref()
                    .map(|idf| self.visit(&idf.clone().into()).unwrap());
            ...
            pub fn visit_StructDefinition(
                ...
                members_by_descending_size
                    .sort_by(|v1, v2| Self::__cmp_type_size(v1, v2).reverse());
                let body = indent(
                    members_by_descending_size
                        .iter()
            ...
            pub fn visit_ContractDefinition(
                ...
                let constructors = ast
                    ...
                    .iter()
                    .map(|e| self.visit(&e.clone().into()).unwrap())
