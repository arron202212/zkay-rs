...
pub fn _check_is_one_of(val: &String, legal_vals:&Vec<String>){
    assert!(legal_vals.contains(val),"Invalid config value {val}, must be one of {:?}",legal_vals);
...
impl UserConfig{
    pub fn new()->Self{
        ...
        let _appdirs: AppInfo = AppInfo{name: "zkay",author:"arron"};
        // User configuration
       ...
       let  _main_crypto_backend = String::from("ecdh-chaskey");
        let _main_crypto_backend_values =lc_string_vec![null, "dummy", "dummy-hom", "rsa-pkcs1.5", "rsa-oaep", "ecdh-aes", "ecdh-chaskey", "paillier", "elgamal"];
        let _addhom_crypto_backend =String::from("elgamal");
        let _addhom_crypto_backend_values = lc_string_vec![null, "dummy-hom", "paillier", "elgamal"];

        // Global defaults
       Self { _proving_scheme: String::from("groth16"),
        ...
        _crypto_backends: BTreeMap::from(
            [(String::from("NON_HOMOMORPHIC"), _main_crypto_backend.clone()),
            (String::from("ADDITIVE"), _addhom_crypto_backend.clone())]
        ),
        _crypto_backend_values: BTreeMap::from(
            [(String::from("NON_HOMOMORPHIC"), _main_crypto_backend_values.clone()),
            (String::from("ADDITIVE"), _addhom_crypto_backend_values.clone()),]
        ),
        ...
        _blockchain_backend_values : lc_vec_s!["w3-eth-tester", "w3-ganache", "w3-ipc", "w3-websocket", "w3-http", "w3-custom"],
        ...
        _data_dir : get_app_dir(AppDataType::UserData, &_appdirs, "data").expect("").to_str().expect("").to_string(),
        _log_dir: get_app_dir(AppDataType::UserData, &_appdirs, "log").expect("").to_str().expect("").to_string(),
...
_appdirs,
    ...
    pub fn main_crypto_backend(&self) -> String{
        ...
        self._get_crypto_backend(&String::from("NON_HOMOMORPHIC")).unwrap()
    }
    ...
    pub fn set_crypto_backend(&mut self, hom: &String, val: String){
        _check_is_one_of(&val, &self._crypto_backend_values[hom].iter().filter_map(|s| if s.is_some(){s.clone()}else{None}).collect());
    ...
    pub fn get_crypto_params(&self, hom: &String) -> CryptoParams{
        ...
        assert!(backend_name.is_some(),"No crypto backend set for homomorphism {:?}",hom);
    ...
    pub fn all_crypto_params(&self) -> Vec<CryptoParams>
       { 
        let crypto_backends:Vec<_> = [String::from("NON_HOMOMORPHIC"),String::from("ADDITIVE")].iter().map(|hom| self._get_crypto_backend(hom)).collect();
         crypto_backends.iter().filter_map(|backend| if let Some(backend)=backend{Some(CryptoParams::new(backend.clone()))}else{None}).collect()
        }
    ...
    pub fn blockchain_node_uri(&self) -> Option<String>{//Union[Any, String, None]
    ...
    pub fn set_blockchain_node_uri(&mut self, val: Option<String>){//Union[Any, String, None]
    ...
    pub fn blockchain_default_account(&self) -> Option<String> { //Union[i32, String, None]
