...
use crate::zkay_ast::ast::{Expression, IdentifierExpr, HybridArgumentIdf, Identifier, BuiltinFunction, FunctionCallExpr, HybridArgType};
...
rmap:RemapMapType<K,V>,
    ...
    // @contextmanager
    pub fn remap_scope(&mut self, scope_stmt:)
        // """
    ...
    pub fn get_current(&self, key: K, default=None) -> V
        // """
    ...
    pub fn join_branch(&self, stmt, true_cond_for_other_branch: IdentifierExpr,
        ...
        for (key, val) in true_state
            ...
            if key in false_state and false_state[key].name == val.name
               ...
               { assert! (false_state[key] == val);
                self.rmap[key] = val;}
            else if key not in false_state
