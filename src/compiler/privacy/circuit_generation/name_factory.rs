...
use crate::zkay_ast::ast::{TypeName, HybridArgType, Expression, HybridArgumentIdf};
...
impl BaseNameFactory{
    ...
    pub fn  get_new_name(&mut self, t: TypeName, inc:bool) -> String
        ...
        let name = format!('{self.base_name}{self.count}_{postfix}');
        if inc
            {self.count += 1;}
         name}
}
...
idfs:Vec<Identifier>,
    ...
    pub fn new( base_name: String, arg_type: HybridArgType)->Self{
        ...
        // self.size = 0
        // self.idfs = []
        Self{base_name_factory:BaseNameFactory::new(base_name),
arg_type,
size:0,
idfs:vec![]
        }
