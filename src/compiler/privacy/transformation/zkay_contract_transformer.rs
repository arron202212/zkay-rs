...
use crate::compiler::privacy::transformation::internal_call_transformer::{transform_internal_calls, compute_transitive_circuit_io_sizes};
...
use crate::compiler::privacy::transformation::zkay_transformer::{ZkayVarDeclTransformer, ZkayExpressionTransformer, ZkayCircuitTransformer, 
    ZkayStatementTransformer};
use crate::config::CFG;
use crate::zkay_ast::ast::{Expression, ConstructorOrFunctionDefinition, IdentifierExpr, VariableDeclaration, 
    AnnotatedTypeName, 
...
pub fn transform_ast(ast: SourceUnit) ->(SourceUnit, BTreeMap<ConstructorOrFunctionDefinition, CircuitHelper>)
    // """
...
pub fn new()->Self{
Self{circuits:BTreeMap::new(),
var_decl_trafo : ZkayVarDeclTransformer::new(),}}
    
    pub fn import_contract(cname: &str, su: &SourceUnit, corresponding_circuit: Option<CircuitHelper>)
        // """
        ...
        if corresponding_circuit.is_some()
            {let c_type = ContractTypeName::new([Identifier(cname)]);
    ...
    pub fn create_contract_variable(cname: &str) -> StateVariableDeclaration
        ...
        {let inst_idf = Identifier(cfg.get_contract_var_name(cname));
        let c_type = ContractTypeName([Identifier(cname)]);

        let cast_0_to_c = PrimitiveCastExpr(c_type, NumberLiteralExpr(0));
         StateVariableDeclaration::new(AnnotatedTypeName::new(c_type), vec!["public", "constant"], inst_idf.clone(), cast_0_to_c)
        
}

    pub fn include_verification_contracts(self, su: SourceUnit, c: ContractDefinition) -> Vec<StateVariableDeclaration>
        // """
       ...
       { let mut contract_var_decls = vec![];
        for crypto_params in c.used_crypto_backends
            {let contract_name = cfg.get_pki_contract_name(crypto_params);
            contract_var_decls.push(self.create_contract_variable(contract_name));}

        for f in c.constructor_definitions.iter().chain(c.function_definitions)
           { if f.requires_verification_when_external and f.has_side_effects
               {let  name = cfg.get_verification_contract_name(c.idf.name, f.name);
    ...
    pub fn create_circuit_helper(fct: ConstructorOrFunctionDefinition,
        ...
        // """
        {CircuitHelper::new(fct, global_owners, ZkayExpressionTransformer, ZkayCircuitTransformer, internal_circ)}
    ...
    pub fn transform_contract(self, su: SourceUnit, c: ContractDefinition) -> ContractDefinition
        ...
        // """
{
        let all_fcts = c.constructor_definitions.iter().chain(c.function_definitions);
        ...
        let field_prime_decl = StateVariableDeclaration::new(AnnotatedTypeName.uint_all(), ["public", "constant"],
        ...
        for f in fcts_with_verification
            ...
            if circuit.requires_zk_data_struct()
               ...
               { let zk_data_struct = StructDefinition(Identifier(circuit.zk_data_struct_name), [
        ...
        for f, params in req_ext_fcts.items()
            {let (ext_f, int_f )= self.split_into_external_and_internal_fct(f, params, global_owners);
    ...
    pub fn create_internal_verification_wrapper(self, ast: ConstructorOrFunctionDefinition)
        ...
        stmts.push(RequireStatement(out_start_idx.binop("+", NumberLiteralExpr(circuit.out_size_trans)).binop("<=", out_var.dot("length"))));
        stmts.push(RequireStatement(in_start_idx.binop("+", NumberLiteralExpr(circuit.in_size_trans)).binop("<=", in_var.dot("length"))));
        ...
        for s in circuit.output_idfs
            {deserialize_stmts.push(s.deserialize(cfg.zk_out_name, out_start_idx, offset));
            if isinstance(s.t, CipherText) and s.t.crypto_params.is_symmetric_cipher()
               ...
               { assert s.t.crypto_params in me_key_idx, "Symmetric cipher but did not request me key";
                ...
                deserialize_stmts.push(s.get_loc_expr().index(cipher_payload_len).assign(sender_key));}
            offset += s.t.size_in_uints;}
        if deserialize_stmts
           { stmts.push(StatementList(Comment.comment_wrap_block("Deserialize output values", deserialize_stmts), excluded_from_simulation=True));}
        ...
        if circuit.has_return_var
            {stmts.push(ReturnStatement(TupleExpr([IdentifierExpr(vd.idf.clone()).override(target=vd) for vd in ast.return_var_decls])));}

        ast.body.statements[:] = stmts;}

    pub fn split_into_external_and_internal_fct(self, f: ConstructorOrFunctionDefinition, original_params: Vec<Parameter>,
                                             global_owners: Vec<PrivacyLabelExpr>) -> (ConstructorOrFunctionDefinition,
    ...
    pub fn create_external_wrapper_body(int_fct: ConstructorOrFunctionDefinition, ext_circuit: CircuitHelper,
                                     original_params: Vec<Parameter>, requires_proof: bool) -> Block
        // """
        ...
        // """
       { let priv_args = [p for p in original_params if p.annotated_type.is_cipher()];
        let args_backends = OrderedDict.fromkeys([p.annotated_type.type_name.crypto_params for p in priv_args]);
        let mut stmts = vec![];
        ...
        let in_arr_var = IdentifierExpr(cfg.zk_in_name).as_type(Array(AnnotatedTypeName.uint_all()));
        ...
        if ext_circuit.requested_global_keys
           { // Ensure that me public key is stored starting at in[0]
            ...
            for (key_owner, crypto_params) in keys
                ...
                key_req_stmts.push(in_arr_var.slice(offset, key_len).assign(IdentifierExpr(tmp_key_var.clone()).slice(0, key_len)));
                offset += key_len;
            ...
            {if p.annotated_type.is_cipher()
               { c = p.annotated_type.type_name;
                ...
                if c.crypto_params.is_symmetric_cipher()
                    ...
                    copy_stmts.push(VariableDeclarationStatement(VariableDeclaration([], p.annotated_type.clone(), p.idf.clone(), "memory"), lit));}}}
        if copy_stmts
        ...
        let in_call = if int_fct.return_parameters
            ...
            TupleExpr([IdentifierExpr(vd.idf.clone()) for vd in int_fct.return_var_decls]).assign(internal_call)
        }
        else
           {  ExpressionStatement(internal_call)};
        stmts.push(Comment("Call internal function"));
        stmts.push(in_call);
        stmts.push(Comment());
        ...
        if requires_proof and not cfg.disable_verification
            ...
            stmts.push(StatementList([Comment("Verify zk proof of execution"), verify], excluded_from_simulation=True));}

        // Add return statement at the end if necessary
        if int_fct.return_parameters
            {stmts.push(ReturnStatement(TupleExpr([IdentifierExpr(vd.idf.clone()) for vd in int_fct.return_var_decls])));}
