...
lazy_static! {
       static ref RE: Regex =
  static ref WS_PATTERN: Regex = Regex::new(r"[ \t\r\n\u000C]").unwrap();
  static ref ID_PATTERN : Regex=  Regex::new(r"[a-zA-Z\$_][a-zA-Z0-9\$_]*").unwrap();
...
// """Add an additional address me = msg.sender state variable declaration right before the closing brace of each contract definition."""
{
    ...
    for m in matches {
        insert_indices.push(find_matching_parenthesis(code, m.end()));
    }
    let parts: Vec<_> = [0]
        .iter()
...
pub fn replace_with_surrogate(
    ...
    loop {
        ...
        if has_ph {
            let repl = matches.and_then(|cap| cap.name("repl").map(|repl| repl.as_str()));
            replacement = create_surrogate_string(repl);
        }

        code = code[..search_idx].to_owned()
            + search_pattern.replace(&code[search_idx..], keep_repl_pattern + replacement);
        search_idx += matches.get(0).end() + 1
    }
}
// """
...
pub fn fake_solidity_code(code: &str) -> String {
    ...
    let mut code = replace_with_surrogate(code, STRING_OR_COMMENT_PATTERN, "");
