...
impl CircuitEvaluator {
    ...
    pub fn setWireValue(w: Wire, v: BigInteger) {
        assert!(
            v.signum() >= 0 && v < Config.FIELD_PRIME,
    ...
    pub fn setWireValue(e: LongElement, value: BigInteger, bitwidthPerChunk: i32) {
    ...
    pub fn writeInputFile() {
        ...
        for e in evalSequence.keys() {
            if let WireLabelInstruction(_e) = e
                && (_e.getType() == LabelType.input || _e.getType() == LabelType.nizkinput)
            {
                let id = _e.getWire().getWireId();
                write!(
                    printWriter,
                    id.to_string() + " " + &format!(":x", valueAssignment[id])
                );
            }
        }
    }
