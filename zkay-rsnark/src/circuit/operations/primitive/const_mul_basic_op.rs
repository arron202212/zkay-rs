...
pub fn newConstMulBasicOp(
    ...
    if !inSign {
        constInteger = Util::modulo(constInteger, Config.FIELD_PRIME);
    } else {
        let mut _constInteger = constInteger.negate();
        _constInteger = Util::modulo(_constInteger, Config.FIELD_PRIME);
...
impl BasicOp for Op<ConstMulBasicOp> {
    fn getOpcode(&self) -> String {
        if !inSign {
            ...
            fomrat!(
                "const-mul-neg-{:x}",
                Config.FIELD_PRIME.subtract(constInteger)
            )
        }
    }
