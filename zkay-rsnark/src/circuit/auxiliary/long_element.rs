...
impl LongElement {
    ...
    pub fn new_with_array(bits: WireArray) -> Self {
        let (array, currentMaxValues, currentBitwidth) = if CHUNK_BITWIDTH >= bits.size() {
            ...
            if size % CHUNK_BITWIDTH != 0 {
                bits = bits.adjustLength(size + (CHUNK_BITWIDTH - size % CHUNK_BITWIDTH));
            ...
            for i in 0..array.len() {
                array[i] = WireArray::new(&bits[i * CHUNK_BITWIDTH..(i + 1) * CHUNK_BITWIDTH])
                    .packAsBits();
                if i == array.len() - 1 {
    ...
    pub fn mulOverflowCheck(o: LongElement) -> bool {
        ...
        for i in 0..array.len() {
            for j in 0..o.array.len() {
                newMaxValues[i + j] =
                    newMaxValues[i + j].add(currentMaxValues[i].multiply(o.currentMaxValues[j]));
            }
        }
        for i in 0..length {
    ...
    pub fn align(totalNumChunks: usize) -> Self {
        ...
        currentMaxValues
            .copy_from_slice(&newMaxValues[..totalNumChunks.min(currentMaxValues.len())]);
        ...
        for i in 0..totalNumChunks {
            if newMaxValues[i].bitLength() > CHUNK_BITWIDTH {
                ...
                let mut rem =
                    WireArray::new(&chunkBits[CHUNK_BITWIDTH..newMaxValues[i].bitLength()])
                        .packAsBits();
                if i != totalNumChunks - 1 {
                    newMaxValues[i + 1] = newMaxValues[i]
                        .shiftRight(CHUNK_BITWIDTH)
    ...
    pub fn getBits(totalBitwidth: i32) -> WireArray {
        if bits.is_some() {
            return bits.adjustLength(if totalBitwidth == -1 {
                bits.len()
            } else {
                totalBitwidth
            });
        ...
        if totalBitwidth <= CHUNK_BITWIDTH && totalBitwidth >= 0 {
            let out = array[0].getBitWires(currentMaxValues[0].bitLength());
            return out.adjustLength(totalBitwidth);
        }

        let limit = totalBitwidth;
        let maxVal = getMaxVal(CHUNK_BITWIDTH);
        ...
        let newLength = (getMaxVal(CHUNK_BITWIDTH).bitLength() * 1.0 / CHUNK_BITWIDTH).ceil();
        ...
        while (idx < limit && chunkIndex < newLength) {
            ...
            if newMaxValues[chunkIndex].bitLength() > CHUNK_BITWIDTH {
                ...
                let rem = WireArray::new(
                    &chunkBits[CHUNK_BITWIDTH..newMaxValues[chunkIndex].bitLength()],
                )
                .packAsBits();
                ...
                alignedChunkBits = newArray[chunkIndex].getBitWires(CHUNK_BITWIDTH).asArray();
            }
        ...
        for i in 0..aiVals.len() {
            for j in 0..biVals.len() {
                solution[i + j] = solution[i + j]
                    ...
                    .modulo(Config.get().unwrap().field_prime);
            }
        }
        solution
    }
    ...
    pub fn muxBit(other: LongElement, w: Wire) -> Self {
        let length = zkay_type(array.len(), other.array.len());
        let mut newArray = vec![self.generator.getZeroWire(); length];
        let newMaxValues = vec![BigInteger.ZERO; length];
        for i in 0..length {
            let b1 = if i < array.len() {
                currentMaxValues[i]
            } else {
                BigInteger.ZERO
            };
    ...
    pub fn getConstant(bitwidth_per_chunk: i32) -> Option<BigInteger> {
        ...
        for i in 0..array.len() {
            if !matches!(array[i], ConstantWire) {
                ...
                constants[i] = (array[i]).getConstant();
            }
        }
    ...
    pub fn assertEqualityNaive(a: LongElement) {
        let bits1 = a.getBits(a.getMaxVal(CHUNK_BITWIDTH).bitLength());
        let bits2 = getBits(getMaxVal(CHUNK_BITWIDTH).bitLength());
    ...
    pub fn assertEquality(e: LongElement) {
        ...
        let limit = zkay_type(a1.len(), a2.len());
        ...
        while (i < limit) {
            ...
            while i + step <= limit - 1 {
                ...
                if b1.add(bounds1[i + step].multiply(delta)).bitLength()
                    < Config.get().unwrap().LOG2_FIELD_PRIME - 2
        ...
        // CHUNK_BITWIDTH
        let alignedAuxConstantSmallChunks = Util::split(auxConstant, CHUNK_BITWIDTH);
                ...
                fn evaluate(evaluator: CircuitEvaluator) {
                    ...
                    for i in 0..carries.len() {
                        ...
                        let mut carryValue = auxConstantChunks[i]
                            .add(a)
                            .subtract(b)
                            .subtract(alignedAuxConstantChunks[i])
                            .add(prevCarry);
                        carryValue = carryValue.shiftRight(steps.get(i) * CHUNK_BITWIDTH);
        ...
        for j in 0..carries.len() + 1 {
            ...
            if auxConstantChunks[j]
                ...
                .add(prevBound.compareTo(Config.get().unwrap().field_prime) >= 0)
            {
    ...
    pub fn assertLessThan(&self, other: LongElement) {
        ...
        let length = zkay_type(a1.len(), a2.len());
        ...
        self.generator
            .addOneAssertion(chunk1.isLessThan(chunk2, LongElement.CHUNK_BITWIDTH));
...
impl Add for LongElement {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        ...
        let length = zkay_type(self.array.len(), o.array.len());
...
impl Sub for LongElement {
    ...
    fn sub(self, rhs: Self) -> Self::Output {
        ...
        let result = self
            .generator
            .createLongElementProverWitness(getMaxVal(CHUNK_BITWIDTH).bitLength());

        self.generator.specifyProverWitnessComputation(&{
            struct Prover;
            impl Instruction for Prover {
                pub fn evaluate(evaluator: CircuitEvaluator) {
                    let myValue = evaluator.getWireValue(LongElement.this, CHUNK_BITWIDTH);
                    let otherValue = evaluator.getWireValue(other, CHUNK_BITWIDTH);
...
impl Mul for LongElement {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        ...
        if o.array.len() == 1 || self.array.len() == 1 || self.isConstant(|| o.isConstant()) {
            ...
            for k in 0..length {
                ...
                for i in 0..length {
                    ...
                    coeff =
                        Util::modulo(coeff.multiply(constant), Config.get().unwrap().field_prime);
                }
