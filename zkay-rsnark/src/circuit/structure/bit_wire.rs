...
use crate::circuit::operations::primitive::add_basic_op::{new_add,AddBasicOp};
use crate::circuit::operations::primitive::const_mul_basic_op::{new_const_mul,ConstMulBasicOp};
use crate::circuit::operations::primitive::mul_basic_op::{new_mul,MulBasicOp};
...
impl BitWire {
    ...
    pub fn mul(&self,w: WireType, desc: Vec<String>) -> WireType {
        ...
        let output = if w.instance_of("BitWire") {
            WireType::VariableBit(VariableBitWire::new(self.generator().currentWireId))
        } else {
            WireType::VariableWire(VariableWire::new(self.generator().currentWireId))
        };
        self.generator().currentWireId += 1;
        let op = new_mul(self.clone(), w, output, desc);
    ...
    pub fn mulb(&self,b: BigInteger, desc: Vec<String>) -> WireType {
        ...
        let out = WireType::LinearCombination(LinearCombinationWire::new(self.generator().currentWireId));
        self.generator().currentWireId += 1;
    ...
    pub fn invAsBit(&self,desc: Vec<String>) -> WireType {
        ...
        let out = WireType::LinearCombinationBit(LinearCombinationBitWire::new(self.generator().currentWireId));
        self.generator().currentWireId += 1;
        let op = new_add(vec![Some(self.generator().oneWire.clone()), neg], out, desc);
        //		self.generator().addToEvaluationQueue(op);
        let cachedOutputs = self.generator().addToEvaluationQueue(op);
        if let Some(cachedOutputs) = cachedOutputs {
            self.generator().currentWireId -= 1;
    ...
    pub fn getBits(&self,w: WireType, bitwidth: i32, desc: Vec<String>) -> WireArray {
        return WireArray::new(vec![Some(WireType::Bit(self.clone()))]).adjustLengthi(bitwidth);
    }
}
