...
impl ConstantWire {
    pub fn new(wireId: i32, value: BigInteger) -> Self {
        super(wireId);
        constant = value.modulo(Config.FIELD_PRIME);
    }
    ...
    pub fn isBinary() -> bool {
        return constant.equals(BigInteger.ONE) || constant.equals(BigInteger.ZERO);
    }
    ...
    pub fn mul(w: Wire, desc: Vec<String>) -> Wire {
        if w.instanceof(ConstantWire) {
            return self
                .generator
                .createConstantWire(constant.multiply(w.constant), desc);
        } else {
    ...
    pub fn mul(b: BigInteger, desc: Vec<String>) -> Wire {
        let sign = b.signum() == -1;
        let newConstant = constant.multiply(b).modulo(Config.FIELD_PRIME);
        ...
        out = ConstantWire::new(
            ...
            if !sign {
                ...
                newConstant.subtract(Config.FIELD_PRIME)
            },
        );
    ...
    pub fn or(w: Wire, desc: Vec<String>) -> Wire {
        if let Some(cw) = w.ConstantWire() {
            ...
            return if constant.equals(BigInteger.ZERO && cw.getConstant().equals(BigInteger.ZERO)) {
                self.generator.zeroWire
            } else {
                self.generator.oneWire
            };
