...
impl<T:setBitsConfig> Wire<T>{
    pub fn new(&self,wireId:i32,t:T)->eyre::Result<Self> {
        ...
        Ok(Self{self.generator : CircuitGenerator.getActiveCircuitGenerator(),
wireId,t
        })
	}

	pub fn new_array(&self,bits:WireArray ,t:T)->Self {
		let mut _self=Self {self.generator : CircuitGenerator.getActiveCircuitGenerator(),wireId:-1,t};
	...
	pub fn getBitWires(&self,bitwidth:i32 , desc:Vec<String>)->WireArray {
		...
		if let Some(bitWires) ==bitWires  {
			if bitwidth < bitWires.len() && self.ConstantWire().is_none(){
				...
				println!("\t If self was called for enforcing a bitwidth constraint, you must use restrictBitLengh(), otherwise you can ignore self.");
...
LinearCombinationWire::new(result)
	...
	pub fn isGreaterThanOrEqual(&self,v:i64 , bitwidth:i32 , desc:Vec<String>)->Wire {
		return self.isGreaterThanOrEqual(self.generator.createConstantWire(v, desc), bitwidth, desc);
	}

	pub fn isGreaterThanOrEqual(&self,b:BigInteger , bitwidth:i32 , desc:Vec<String>) ->Wire{
		return self.isGreaterThanOrEqual(self.generator.createConstantWire(b, desc), bitwidth, desc);
	}
...
LinearCombinationWire::new(result)
	...
	pub fn trimBits(&self,currentNumOfBits:i32 , desiredNumofBits:i32 , desc:Vec<String>)->Wire {
...
LinearCombinationWire::new(result)
	 ...
	 fn pack(&self,desc:Vec<String>) {
		if wireId != -1 {
			...
			assert!(bits.is_some(),"A Pack operation is tried on a wire that has no bits.");
