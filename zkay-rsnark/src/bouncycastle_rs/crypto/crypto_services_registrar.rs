...
// 全域屬性存儲 (對應 globalProperties)
static GLOBAL_PROPERTIES: OnceLock<RwLock<HashMap<Property, Vec<Arc<dyn Send + Sync + 'static>>>>> = OnceLock::new();

// 線程局部屬性存儲 (對應 threadProperties)
thread_local! {
    static THREAD_PROPERTIES: RefCell<HashMap<Property, Vec<Arc<dyn Send + Sync + 'static>>>> = RefCell::new(HashMap::new());
}

// 默認隨機數生成器緩存
static DEFAULT_SECURE_RANDOM: OnceLock<Mutex<Box<dyn SecureRandom>>> = OnceLock::new();
pub trait SecureRandom: Send + Sync {
    fn fill_bytes(&mut self, dest: &mut [u8]);
}

// 實現一個基於系統隨機源的默認實例
struct DefaultRng(OsRng);
...
impl CryptoServicesRegistrar {
    /// 獲取全局默認的隨機數生成器
    pub fn get_secure_random() -> &'static Mutex<Box<dyn SecureRandom>> {
        DEFAULT_SECURE_RANDOM.get_or_init(|| {
            Mutex::new(Box::new(DefaultRng(OsRng)))
        })
    }

    /// 設置全局默認的隨機數生成器 (需檢查權限)
    pub fn set_secure_random(rng: Box<dyn SecureRandom>) {
        // 在 Rust 中這裡可以實施自定義的權限檢查邏輯
        if let Some(lock) = DEFAULT_SECURE_RANDOM.get() {
            let mut guard = lock.lock().unwrap();
            *guard = rng;
        } else {
            let _ = DEFAULT_SECURE_RANDOM.set(Mutex::new(rng));
        }
    }
}
impl CryptoServicesRegistrar {
    fn init_default_params() {
        let p_512 = BigInt::parse_bytes(b"fca682ce8e12caba...", 16).unwrap();
        let q_512 = BigInt::parse_bytes(b"962eddcc369cba8ebb260ee6b6a126d9346e38c5", 16).unwrap();
        let g_512 = BigInt::parse_bytes(b"678471b27a9cf44ee91a49c5147db1a9...", 16).unwrap();

        // 構造 DSAParameters 對象 (假設已定義該結構)
        let dsa_params = Arc::new(DsaParameters::new(p_512, q_512, g_512));

        Self::set_global_property(Property::DsaDefaultParams, vec![dsa_params]);
    }

    pub fn get_property<T: 'static>(property: Property) -> Option<Arc<T>> {
        // 先看線程局部，再看全局
        let thread_val = THREAD_PROPERTIES.with(|tp| {
            tp.borrow().get(&property).and_then(|v| v.first().cloned())
        });

        if let Some(val) = thread_val {
            return val.downcast::<T>().ok();
        }

        let global_map = GLOBAL_PROPERTIES.get()?.read().unwrap();
        global_map.get(&property).and_then(|v| v.first().cloned())?.downcast::<T>().ok()
    }
}
