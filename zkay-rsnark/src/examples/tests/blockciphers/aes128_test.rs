...
for sboxOption in AES128CipherGadget.SBoxOption.values()
			...
			CircuitGenerator generator = CircuitGenerator::new("AES128_Test1_"+sboxOption) {
				 ...
				 Vec<Option<WireType>> plaintext; // 16 bytes
				 Vec<Option<WireType>> key; // 16 bytes
				 Vec<Option<WireType>> ciphertext; // 16 bytes
...
let msgArray = msgV.toByteArray();
					msgArray = Arrays.copyOfRange(msgArray, msgArray.len() - 16,
							msgArray.len());
					keyArray = Arrays.copyOfRange(keyArray, keyArray.len() - 16,
							keyArray.len());
...
let resultArray = result.toByteArray();
			resultArray = Arrays.copyOfRange(resultArray, resultArray.len() - 16,
					resultArray.len());
