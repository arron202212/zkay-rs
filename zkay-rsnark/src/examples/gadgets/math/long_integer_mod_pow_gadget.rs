...
impl Gadget for LongIntegerModPowGadget{
	  fn buildCircuit() {
		let one = LongElement::new(vec![BigInteger::default();] {BigInteger.ONE});
		...
		for i in (0..=eBits.length - 1).rev()
			...
			let squareModM = LongIntegerModGadget::new(square, m, mMinBits, false, "modPow: prod^2 mod m").getRemainder();
