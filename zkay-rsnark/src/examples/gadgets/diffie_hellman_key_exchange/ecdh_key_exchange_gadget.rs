...
pub struct ECDHKeyExchangeGadget  {
	...
	// gadget outputs
	 ...
	 sharedSecret:Wire, // the x-coordinate of the derived key ((this
								// party's secret)*H).x
...
// Note: this parameterization assumes that the underlying field has
	...
	pub  ECDHKeyExchangeGadget(baseX:Wire, hX:Wire, secretBits:Vec<Wire>,
			desc:Vec<String>) {
		super(desc);
...
impl Gadget for ECDHKeyExchangeGadget{
	...
	pub  fn new(baseX:Wire, baseY:Wire, hX:Wire, hY:Wire,
	...
	pub fn getOutputPublicValue()-> Wire {
		return outputPublicValue;
	}

	pub fn getSharedSecret()-> Wire {
		return sharedSecret;
	}
}
