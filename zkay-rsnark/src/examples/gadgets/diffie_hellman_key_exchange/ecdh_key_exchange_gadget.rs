...
pub struct ECDHKeyExchangeGadget extends Gadget {
	...
	// The Affine point representation is used as it saves one gate per bit
	 ...
	 AffinePoint hPoint; // H is the other party's pub  value
								// H = (other party's secret)* Base <- scalar EC
								// multiplication

	 Vec<Wire> secretBits; // the bits of the secret generated by this party
								// (follows little-endian order)

	// gadget outputs
	 Wire outputPublicValue; // the x-coordinate of the key exchange
									// material to be sent to the other party
									// outputPublicValue = ((this party's
									// secret)*Base).x

	 Wire sharedSecret; // the x-coordinate of the derived key ((this
								// party's secret)*H).x

	 Vec<AffinePoint> baseTable;
	 Vec<AffinePoint> hTable;
	...
	pub  ECDHKeyExchangeGadget(Wire baseX, Wire hX, secretBits:Vec<Wire>,
			desc:Vec<String>) {
		super(desc);
	...
	pub  ECDHKeyExchangeGadget(Wire baseX, Wire baseY, Wire hX, Wire hY,
			secretBits:Vec<Wire>, desc:Vec<String>) {
		super(desc);
	...
	// (not as input to the gadget)
	  assertValidPointOnEC(Wire x, Wire y) {
		Wire ySqr = y.mul(y);
		Wire xSqr = x.mul(x);
	 ...
	 Vec<AffinePoint> preprocess(AffinePoint p) {
		Vec<AffinePoint> precomputedTable = vec![AffinePoint::default();secretBits.length];
	 ...
	 * conditions above)
	 */
	 AffinePoint mul(AffinePoint p, secretBits:Vec<Wire>,
			precomputedTable:Vec<AffinePoint>) {

		AffinePoint result = AffinePoint::new(
	 ...
	 AffinePoint doubleAffinePoint(AffinePoint p) {
		...
		Wire l1 = FieldDivisionGadget::new(x_2.mul(3)
	 ...
	 AffinePoint addAffinePoints(AffinePoint p1, AffinePoint p2) {
		...
		Wire q = FieldDivisionGadget::new(diffY, diffX).getOutputWires()[0];
		Wire q2 = q.mul(q);
		Wire q3 = q2.mul(q);
	...
	pub   BigInteger computeYCoordinate(BigInteger x) {
	...
	pub   validateInputs() {
	  ...
	  assertPointOrder(AffinePoint p, table:Vec<AffinePoint>) {

		Wire o = generator.createConstantWire(SUBGROUP_ORDER);
		Vec<Wire> bits = o.getBitWires(SUBGROUP_ORDER.bitLength()).asArray();

		AffinePoint result = AffinePoint::new(table[bits.length - 1]);
	...
	pub  Wire getOutputPublicValue() {
		return outputPublicValue;
	}

	pub  Wire getSharedSecret() {
		return sharedSecret;
	}
}
