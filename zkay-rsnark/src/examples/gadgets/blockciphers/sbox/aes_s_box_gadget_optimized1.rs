...
impl Gadget for AESSBoxGadgetOptimized1 {
    pub fn solveLinearSystems() {
        ...
        for i in 0..=15 {
            ...
            let polyCoeffs = vec![BigInteger.ONE];
            ...
            for k in 0..mat.length {
                ...
                for j in 1..=16 {
                    mat[k][j] = p.multiply(mat[k][j - 1]).modulo(Config.FIELD_PRIME);
                }
                ...
                polyCoeffs = polyMul(
                    polyCoeffs,
                    vec![Config.FIELD_PRIME.subtract(p), BigInteger.ONE],
                );
            }
    ...
    fn checkIfProverCanCheat(mat: Vec<Vec<BigInteger>>, valueSet: HashSet<Integer>) -> bool {
        ...
        for k in 0..256 * 256 {
            ...
            for i in 1..16 {
                ...
                p = p.multiply(BigInteger.valueOf(k)).modulo(Config.FIELD_PRIME);
            }
