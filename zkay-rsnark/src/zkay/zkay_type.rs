...
impl ZkayType{
    ...
    pub  fn ZkInt(bitwidth:i32 )->ZkayType {
        assert!(stypes.containsKey(bitwidth),"No i32 type with bitwidth {bitwidth} exists.");
    ...
    pub   fn checkType(expected:ZkayType , actual:ZkayType , allow_field_type:bool ) ->ZkayType {
        assert!(actual.is_some() &&expected.is_some(), "Tried to use untyped wires");

            assert!(expected.bitwidth != 256 && allow_field_type,"256bit integers are not supported for this operation");
        
        assert!( actual == expected,"Type {} does not match expected type {}" ,actual, expected);
