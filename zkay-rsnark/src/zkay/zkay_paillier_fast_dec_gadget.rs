...
impl ZkayPaillierFastDecGadget{
	pub  ZkayPaillierFastDecGadget(n:LongElement , nBits:i32 , lambda:LongElement , cipher:LongElement , desc:Vec<String>) {
		super(desc);
		self.n = n;
		self.nBits = nBits;
...
impl Gadget for ZkayPaillierFastDecGadget{
	  fn buildCircuit() {
		...
		let lambdaInverse = LongIntegerModInverseGadget::new(lambda, n, false, "lambda^(-1)").getResult();

		// plain = L(cipher^lambda mod n^2) / lambda mod n
		let cPowLambda = LongIntegerModPowGadget::new(cipher, lambda, nSquare, nSquareMinBits, "c^lambda").getResult();
		let lOutput = LongIntegerFloorDivGadget::new(cPowLambda.subtract(1), n, "(c^lambda - 1) / n").getQuotient();
