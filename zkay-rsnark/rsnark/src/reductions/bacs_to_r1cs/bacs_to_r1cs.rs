...
pub fn bacs_to_r1cs_instance_map<FieldT: FieldTConfig>(
    ...
    for g in &circuit.gates {
        result.constraints.push(r1cs_constraint::<FieldT>::new(
            g.lhs.clone(),
            g.rhs.clone(),
            linear_combination::<FieldT>::new3(g.output.clone()),
        ));
        // #ifdef DEBUG
    ...
    for g in &circuit.gates {
        if g.is_circuit_output {
            result.constraints.push(r1cs_constraint::<FieldT>::new(
                linear_combination::<FieldT>::new(1),
                linear_combination::<FieldT>::new3(g.output.clone()),
                linear_combination::<FieldT>::new(0),
            ));
...
pub fn bacs_to_r1cs_witness_map<FieldT: FieldTConfig>(
    circuit: &bacs_circuit<FieldT>,
    primary_input: &bacs_primary_input<FieldT>,
    auxiliary_input: &bacs_auxiliary_input<FieldT>,
