...
pub struct sp_compliance_step_pcd_circuit_maker<ppT> {
    ...
    unpack_sp_translation_step_vk_and_incoming_message_payload_digests:
        Vec<multipacking_gadget<FieldT>>,
...
impl sp_compliance_step_pcd_circuit_maker<ppT> {
    pub fn new(compliance_predicate: &r1cs_pcd_compliance_predicate<FieldT>) -> Self {
        ...
        unpack_outgoing_message = RcCell::new(multipacking_gadget::<FieldT>(
            &pb,
        ...
        sp_translation_step_vk_and_incoming_message_payload_digests
            .resize(compliance_predicate_arity);
        for i in 0..compliance_predicate_arity {
        ...
        for i in 0..compliance_predicate_arity {
            blocks_for_incoming_messages.push(block_variable::<FieldT>(
                &pb,
        ...
        sp_translation_step_vk = RcCell::new(r1cs_ppzksnark_verification_key_variable::<ppT>::new(
            pb,
            sp_translation_step_vk_bits,
            sp_translation_step_pcd_circuit_maker::<other_curve<ppT>>::input_size_in_elts(),
        ...
        for i in 0..compliance_predicate_arity {
            ...
            verifiers.push(r1cs_ppzksnark_verifier_gadget::<ppT>(
                &pb,
                *sp_translation_step_vk,
                verifier_input[i],
    ...
    pub fn generate_r1cs_constraints() {
        ...
        PROFILE_CONSTRAINTS(&pb, "miscellaneous");
        {
            ...
            pb.add_r1cs_constraint(
                r1cs_constraint::<FieldT>(incoming_message_types[0], 1 - verification_result, 0),
...
impl sp_translation_step_pcd_circuit_maker<ppT> {
    pub fn new(sp_compliance_step_vk: &r1cs_ppzksnark_verification_key<other_curve<ppT>>) {
        ...
        /* prepare arguments for the verifier */
        hardcoded_sp_compliance_step_vk = RcCell::new(
    ...
    pub fn generate_r1cs_witness(
        sp_translation_step_input: r1cs_primary_input<ffec::Fr<ppT>>,
        compliance_step_proof: &r1cs_ppzksnark_proof<other_curve<ppT>>,
    ) {
        self.pb.clear_values();
        sp_translation_step_pcd_circuit_input
            .fill_with_field_elements(&pb, sp_translation_step_input);
        ...
        for i in 0..self.pb.num_inputs() {
            self.pb
                .borrow()
                .val(&variable::<FieldT, pb_variable>(i + 1))
                .print();
        }
...
pub fn get_sp_compliance_step_pcd_circuit_input<ppT>(
...
pub fn get_sp_translation_step_pcd_circuit_input<ppT>(
    sp_translation_step_vk_bits: &bit_vector,
    primary_input: &r1cs_pcd_compliance_predicate_primary_input<ffec::Fr<other_curve<ppT>>>,
) -> r1cs_primary_input<ffec::Fr<ppT>> {
