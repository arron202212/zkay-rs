...
pub struct mp_translation_step_pcd_circuit_maker<ppT> {
    ...
    hardcoded_compliance_step_vk:
        RcCell<r1cs_ppzksnark_preprocessed_r1cs_ppzksnark_verification_key_variable<ppT>>,
...
impl mp_compliance_step_pcd_circuit_maker<ppT> {
    pub fn new(
        ...
        let commitment_size =
            set_commitment_gadget::<FieldT, CRH_with_bit_out_gadget<FieldT>>::root_size_in_bits();

        let output_block_size = commitment_size + outgoing_msg_size_in_bits;
        let max_incoming_payload_length = *std::max_element(
        ...
        CRH_with_bit_out_gadge::<FieldT>::sample_randomness(std::max(
            output_block_size,
        ...
        for i in 0..compliance_predicate.max_arity {
            ...
            incoming_message_payloads[i].allocate(
                ...
                FMT("", "incoming_message_payloads_{}", i),
            );

            incoming_message_vars[i]
                .insert(incoming_message_vars[i].end(), incoming_message_types[i]);
        ...
        compliance_predicate_as_gadget = RcCell::new(gadget_from_r1cs::<FieldT>::new(
            pb,
            [
        ...
        unpack_outgoing_message = RcCell::new(multipacking_gadget::<FieldT>::new(
            pb,
        ...
        for i in 0..compliance_predicate.max_arity {
            ...
            incoming_messages_bits[i].allocate(
                ...
                FMT("", "incoming_messages_bits_{}", i),
            );
            unpack_incoming_messages.push(multipacking_gadget::<FieldT>(
                &pb,
                incoming_messages_bits[i],
                incoming_message_vars[i],
                field_logsize(),
                FMT("", "unpack_incoming_messages_{}", i),
            ));
        }
        ...
        for i in 0..compliance_predicate.max_arity {
            commitment_and_incoming_message_digests[i].allocate(
                &pb,
                digest_size,
                FMT("", "commitment_and_incoming_message_digests_{}", i),
            );
        }

        /* allocate commitment, verification key(s) and membership checker(s)/proof(s) */
        commitment = RcCell::new(set_commitment_variable::<
            FieldT,
            CRH_with_bit_out_gadget<FieldT>,
        >::new(pb, commitment_size, "commitment"));
        ...
        if compliance_predicate.relies_on_same_type_inputs {
            ...
            membership_proofs.push(
                set_membership_proof_variable < FieldT,
                CRH_with_bit_out_gadget
                    < FieldT >> (&pb, max_number_of_predicates, "membership_proof"),
            );
            membership_checkers.push(
                set_commitment_gadget < FieldT,
                CRH_with_bit_out_gadget
                    < FieldT
                        >> (
                            &pb,
            ...
            for i in 0..compliance_predicate.max_arity {
                translation_step_vks_bits[i].allocate(
                    &pb,
                    translation_step_vk_size_in_bits,
                    FMT("", "translation_step_vks_bits_{}", i),
                );
        ...
        /* allocate blocks */
        block_for_outgoing_message = RcCell::new(block_variable::<FieldT>::new(
            pb,
        ...
        for i in 0..compliance_predicate.max_arity {
            block_for_incoming_messages.push(block_variable::<FieldT>(
                &pb,
                [commitment.bits, incoming_messages_bits[i]],
                FMT("", "block_for_incoming_messages_{}", i),
            ));
        }

        /* allocate hash checkers */
        hash_outgoing_message = RcCell::new(CRH_with_field_out_gadget::<FieldT>::new(
            pb,
            output_block_size,
            *block_for_outgoing_message,
            mp_compliance_step_pcd_circuit_input,
        ...
        for i in 0..compliance_predicate.max_arity {
            ...
            hash_incoming_messages.push(CRH_with_field_out_gadget::<FieldT>(
                &pb,
                input_block_size,
                block_for_incoming_messages[i],
                commitment_and_incoming_message_digests[i],
                FMT("", "hash_incoming_messages_{}", i),
            ));
        }
        ...
        if compliance_predicate.relies_on_same_type_inputs {
            translation_step_vks.push(r1cs_ppzksnark_verification_key_variable::<ppT>(
                &pb,
                translation_step_vks_bits[0],
                mp_translation_step_pcd_circuit_maker::<other_curve<ppT>>::input_size_in_elts(),
                "translation_step_vk",
        ...
        for i in 0..compliance_predicate.max_arity {
            commitment_and_incoming_messages_digest_bits[i].allocate(
                ...
                FMT("", "commitment_and_incoming_messages_digest_bits_{}", i),
            );
            unpack_commitment_and_incoming_message_digests.push(multipacking_gadget::<FieldT>(
                &pb,
            ...
            proof.push(r1cs_ppzksnark_proof_variable::<ppT>(
                &pb,
                FMT("", "proof_{}", i),
            ));
            ...
            verifier.push(r1cs_ppzksnark_verifier_gadget::<ppT>(
                &pb,
                vk_to_be_used,
                verifier_input[i],
        ...
        PROFILE_CONSTRAINTS(&pb, "miscellaneous");
        {
            ...
            );
            {
                for i in 0..compliance_predicate.max_arity {
                    pb.add_r1cs_constraint(
                        ...
                        FMT("", "not_base_case_implies_valid_proof_{}", i),
                    );
                }
            }
                ...
                );
                {
                    for i in 0..compliance_predicate.max_arity {
                        pb.add_r1cs_constraint(
                            ...
                            FMT("", "non_base_types_equal_{}", i),
                        );
                    }

                    pb.add_r1cs_constraint(
                        r1cs_constraint::<FieldT>(common_type, 1 - membership_check_results[0], 0),
                    ...
                    for (i, it) in compliance_predicate.accepted_input_types.iter().enumerate() {
                        pb.add_r1cs_constraint(
                            r1cs_constraint::<FieldT>(
                                ...
                                (if i == compliance_predicate.accepted_input_types.len() - 1 {
                                    0 * ONE
                                } else {
                ...
                PROFILE_CONSTRAINTS(&pb, "check that all s messages have validly selected VKs");
                {
                    for i in 0..compliance_predicate.max_arity {
                        pb.add_r1cs_constraint(
                            ...
                            FMT("", "not_base_case_implies_valid_vk_{}", i),
                        );
                    }
                }
            }
            pb.add_r1cs_constraint(
                r1cs_constraint::<FieldT>(
                    ...
                    FieldT(compliance_predicate.types),
                ),
    ...
    pub fn generate_r1cs_witness(
        ...
        compliance_predicate_as_gadget.generate_r1cs_witness(
            ...
            compliance_predicate_auxiliary_input
                .as_r1cs_auxiliary_input(compliance_predicate.incoming_message_payload_lengths),
        );
        ...
        if compliance_predicate.relies_on_same_type_inputs {
            ...
            for i in 0..compliance_predicate.max_arity {
                ...
                if self.pb.borrow().val(&common_type).is_zero() {
                    self.pb.borrow().val(&common_type) =
                        self.pb.borrow().val(&incoming_message_types[i]);
                    nonzero_type_idx = i;
                } else {
                    assert!(
                        self.pb.borrow().val(&common_type)
            ...
            self.pb.borrow().val(&membership_check_results[0]) =
                if self.pb.borrow().val(&common_type).is_zero() {
                    FieldT::zero()
                } else {
                    FieldT::one()
            ...
            for (i, it) in compliance_predicate.accepted_input_types.iter().enumerate() {
                pb.borrow().val(&common_type_check_aux[i]) = ((if i == 0? {
                    pb.borrow().val(&common_type)
                } else {
                    pb.borrow().val(&common_type_check_aux[i - 1])
                }) * (pb.borrow().val(&common_type)
                    - FieldT(*it)));
            }
        } else {
            for i in 0..membership_checkers.len() {
                self.pb.borrow().val(&membership_check_results[i]) =
                    (if self.pb.borrow().val(&incoming_message_types[i]).is_zero() {
                        FieldT::zero()
                    } else {
                        FieldT::one()
                    });
...
impl mp_translation_step_pcd_circuit_maker<ppT> {
    pub fn new(compliance_step_vk: r1cs_ppzksnark_verification_key<other_curve<ppT>>) -> Self {
        ...
        unpack_mp_translation_step_pcd_circuit_input =
            RcCell::new(multipacking_gadget::<FieldT>::new(
                pb,
                unpacked_mp_translation_step_pcd_circuit_input,
                mp_translation_step_pcd_circuit_input,
                field_capacity(),
        ...
        /* prepare arguments for the verifier */
        hardcoded_compliance_step_vk = RcCell::new(
        ...
        online_verifier = RcCell::new(r1cs_ppzksnark_online_verifier_gadget::<ppT>::new(
            pb,
            *hardcoded_compliance_step_vk,
            unpacked_mp_translation_step_pcd_circuit_input,
    ...
    pub fn generate_r1cs_witness(
        translation_step_input: r1cs_primary_input<ffec::Fr<ppT>>,
...
pub fn get_mp_compliance_step_pcd_circuit_input(
    commitment_to_translation_step_r1cs_vks: set_commitment,
    primary_input: r1cs_pcd_compliance_predicate_primary_input<ffec::Fr<ppT>>,
) -> r1cs_primary_input<ffec::Fr<ppT>> {
...
pub fn get_mp_translation_step_pcd_circuit_input(
    commitment_to_translation_step_r1cs_vks: set_commitment,
    primary_input: r1cs_pcd_compliance_predicate_primary_input<ffec::Fr<other_curve<ppT>>>,
) -> r1cs_primary_input<ffec::Fr<ppT>> {
