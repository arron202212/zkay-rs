...
impl r1cs_ppzkadsnark_proving_key<ppT> {
    ...
    pub fn G1_sparse_size(&self) -> usize {
        return 2 * (A_query.len() + C_query.len())
            + B_query.len()
            + H_query.len()
            + K_query.len()
            + 1;
    ...
    pub fn size_in_bits(&self) -> usize {
        return A_query.size_in_bits()
            ...
            + size_in_bits(H_query)
            + size_in_bits(K_query)
...
impl r1cs_ppzkadsnark_verification_key<ppT> {
    ...
    pub fn size_in_bits(&self) -> usize {
        return G1_size() * G1::<snark_pp<ppT>>::size_in_bits()
            + G2_size() * G2::<snark_pp<ppT>>::size_in_bits(); // possible zksnark bug
    }
...
impl r1cs_ppzkadsnark_proof<ppT> {
    ...
    pub fn size_in_bits(&self) -> usize {
        return G1_size() * G1::<snark_pp<ppT>>::size_in_bits()
            + G2_size() * G2::<snark_pp<ppT>>::size_in_bits();
    }
    ...
    pub fn is_well_formed(&self) -> bool {
        return (g_A.g.is_well_formed()
            ...
            && g_H.is_well_formed()
            && g_K.is_well_formed()
            && g_Aau.g.is_well_formed()
            && g_Aau.h.is_well_formed()
            && muA.is_well_formed());
    }
...
pub fn r1cs_ppzkadsnark_auth_generator<ppT: PptConfig>() -> r1cs_ppzkadsnark_auth_keys<ppT> {
    ...
    let mut minusI2 = G2::<snark_pp<ppT>>::zero() - i * G2::<snark_pp<ppT>>::one();
...
pub fn r1cs_ppzkadsnark_auth_sign<ppT: PptConfig>(
    ...
    for i in 0..ins.len() {
        ...
        let mut val = r1cs_ppzkadsnark_auth_data::<ppT>::new((lambda + sk.i * ins[i]), Lambda, sig);
        res.push(val);
    }
...
pub fn r1cs_ppzkadsnark_generator<ppT: PptConfig,const NN:usize>(
    ...
    let mut qap_inst = r1cs_to_qap_instance_map_with_evaluation(cs_copy, t);
    ...
    let mut vk = r1cs_ppzkadsnark_verification_key::<ppT>(
        alphaA_g2,
        alphaB_g1,
        alphaC_g2,
        gamma_g2,
    ...
    let mut pk = r1cs_ppzkadsnark_proving_key::<ppT>(
...
pub fn r1cs_ppzkadsnark_prover<ppT>(
    pk: &r1cs_ppzkadsnark_proving_key<ppT>,
    primary_input: &r1cs_ppzkadsnark_primary_input<ppT>,
    auxiliary_input: &r1cs_ppzkadsnark_auxiliary_input<ppT>,
    auth_data: &Vec<r1cs_ppzkadsnark_auth_data<ppT>>,
    ...
    g_H = g_H
        + multi_exp::<G1<snark_pp<ppT>>, Fr<snark_pp<ppT>>, multi_exp_method_BDLO12>(
            pk.H_query.begin(),
    ...
    muA = muA
        + multi_exp::<G1<snark_pp<ppT>>, Fr<snark_pp<ppT>>, multi_exp_method_bos_coster>(
            Ains.begin(),
    ...
    let mut proof = r1cs_ppzkadsnark_proof::<ppT>(g_A, g_B, g_C, g_H, g_K, g_Ain, muA);
    proof.print_size();
...
pub fn r1cs_ppzkadsnark_online_verifier<ppT>(
...
pub fn r1cs_ppzkadsnark_verifier<ppT>(
    vk: &r1cs_ppzkadsnark_verification_key<ppT>,
    proof: &r1cs_ppzkadsnark_proof<ppT>,
    sak: &r1cs_ppzkadsnark_sec_auth_key<ppT>,
    labels: &Vec<labelT>,
...
pub fn r1cs_ppzkadsnark_online_verifier2<ppT>(
    pvk: &r1cs_ppzkadsnark_processed_verification_key<ppT>,
    auth_data: &Vec<r1cs_ppzkadsnark_auth_data<ppT>>,
    proof: &r1cs_ppzkadsnark_proof<ppT>,
    pak: &r1cs_ppzkadsnark_pub_auth_key<ppT>,
...
pub fn r1cs_ppzkadsnark_verifier2<ppT>(
    vk: &r1cs_ppzkadsnark_verification_key<ppT>,
    auth_data: &Vec<r1cs_ppzkadsnark_auth_data<ppT>>,
    proof: &r1cs_ppzkadsnark_proof<ppT>,
    pak: &r1cs_ppzkadsnark_pub_auth_key<ppT>,
