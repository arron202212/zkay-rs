...
pub fn test_r1cs_se_ppzksnark<ppT: ppTConfig>(num_constraints: usize, input_size: usize) {
    print_header("(enter) Test R1CS SEppzkSNARK");
    ...
    let mut bit = run_r1cs_se_ppzksnark::<ppT>(&example, test_serialization);
    assert!(bit);

    print_header("(leave) Test R1CS SEppzkSNARK");
}

fn main<default_r1cs_se_ppzksnark_pp: ppTConfig>() -> i32 {
