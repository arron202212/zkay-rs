...
impl<ppT> r1cs_se_ppzksnark_proving_key {
    ...
    pub fn G1_size(&self) -> usize {
        A_query.len() + C_query_1.len() + C_query_2.len() + 3 + G_gamma2_Z_t.len()
    }
    ...
    pub fn size_in_bits(&self) -> usize {
        G1_size() * ppT::G1::size_in_bits() + G2_size() * ppT::G2::size_in_bits()
    }
...
impl<ppT> r1cs_se_ppzksnark_proof<ppT> {
    ...
    // {}
    pub fn new(A: ffec::G1<ppT>, B: ffec::G2<ppT>, C: ffec::G1<ppT>) -> Self {
        Self { A, B, C }
    }
 ...
 */
pub fn r1cs_se_ppzksnark_prover<ppT>(
    pk: &r1cs_se_ppzksnark_proving_key<ppT>,
 ...
 */
pub fn r1cs_se_ppzksnark_verifier_weak_IC<ppT>(
    vk: &r1cs_se_ppzksnark_verification_key<ppT>,
    primary_input: &r1cs_se_ppzksnark_primary_input<ppT>,
 ...
 */
pub fn r1cs_se_ppzksnark_verifier_strong_IC<ppT>(
    vk: &r1cs_se_ppzksnark_verification_key<ppT>,
    primary_input: &r1cs_se_ppzksnark_primary_input<ppT>,
    proof: &r1cs_se_ppzksnark_proof<ppT>,
) -> bool {
 ...
 */
pub fn r1cs_se_ppzksnark_verifier_process_vk<ppT>(
    vk: &r1cs_se_ppzksnark_verification_key<ppT>,
 ...
 * (2) has weak input consistency.
 */
fn r1cs_se_ppzksnark_online_verifier_weak_IC<ppT>(
    ...
    let G_psi = pvk.query[0]
        + ffec::multi_exp::<ffec::G1<ppT>, ffec::Fr<ppT>, ffec::multi_exp_method_bos_coster>(
            pvk.query.begin() + 1,
            pvk.query.end(),
            primary_input.begin(),
            primary_input.end(),
    ...
    let test1_l = ppT::miller_loop(
        ppT::precompute_G1(proof.A + pvk.G_alpha),
 ...
 * (2) has strong input consistency.
 */
pub fn r1cs_se_ppzksnark_online_verifier_strong_IC<ppT>(
    pvk: &r1cs_se_ppzksnark_processed_verification_key<ppT>,
    primary_input: &r1cs_se_ppzksnark_primary_input<ppT>,
    &proof: r1cs_se_ppzksnark_proof<ppT>,
) -> bool {
...
impl<ppT> fmt::Display for r1cs_se_ppzksnark_verification_key<ppT> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            vk.H, vk.G_alpha, vk.H_beta, vk.G_gamma, vk.H_gamma, vk.query,
        )
    }
}
...
impl<ppT> fmt::Display for r1cs_se_ppzksnark_proof<ppT> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}{}{}", proof.A, proof.B, proof.C,)
    }
}
...
pub fn r1cs_se_ppzksnark_generator<ppT>(
    ...
    letH_gamma_table = get_window_table(ppT::Fr::size_in_bits(), H_gamma_window, H_gamma);
    ...
    let verifier_query = ffec::batch_exp::<ffec::G1<ppT>, ffec::Fr<ppT>>(
        ppT::Fr::size_in_bits(),
        G_window,
        G_table,
        tmp_exponents,
    ...
    let A_query = ffec::batch_exp::<ffec::G1<ppT>, ffec::Fr<ppT>>(
        ppT::Fr::size_in_bits(),
        G_window,
        G_table,
        tmp_exponents,
    ...
    let B_query = ffec::batch_exp::<ffec::G2<ppT>, ffec::Fr<ppT>>(
        ppT::Fr::size_in_bits(),
        H_gamma_window,
        H_gamma_table,
        At,
    ...
    let G_gamma2_Z_t = ffec::batch_exp::<ffec::G1<ppT>, ffec::Fr<ppT>>(
        ppT::Fr::size_in_bits(),
        G_window,
        G_table,
        tmp_exponents,
    ...
    for i in sap_inst..=sap_inst.num_variables() {
        tmp_exponents.push(gamma * (gamma * Ct[i] + (alpha + beta) * At[i]));
    }
    let C_query_1 = ffec::batch_exp::<ppT::G1, ppT::Fr>(
        ppT::Fr::size_in_bits(),
        G_window,
        G_table,
        tmp_exponents,
    ...
    let C_query_2 = ffec::batch_exp::<ppT::G1, ppT::Fr>(
        ppT::Fr::size_in_bits(),
        G_window,
        G_table,
        tmp_exponents,
    ...
    let vk = r1cs_se_ppzksnark_verification_key::<ppT>(
        H,
        G_alpha,
        H_beta,
        G_gamma,
