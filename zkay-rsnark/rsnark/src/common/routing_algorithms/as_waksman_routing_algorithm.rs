 ...
 */
pub type as_waksman_topology = Vec<Vec<(usize, usize)>>;
 ...
 */
pub fn as_waksman_switch_output(
    num_packets: usize,
    row_offset: usize,
    row_idx: usize,
    use_top: bool,
    ...
    return row_offset
        ...
        + (if use_top {
            ...
            as_waksman_top_height(num_packets)
        });
}
 ...
 */
pub fn as_waksman_switch_input(
    num_packets: usize,
    row_offset: usize,
    row_idx: usize,
    use_top: bool,
...
pub fn as_waksman_num_columns(num_packets: usize) -> usize {
    return if num_packets > 1 {
        2 * log2(num_packets) - 1
    } else {
        0
    };
}
 ...
 */
pub fn construct_as_waksman_inner(
    left: usize,
    right: usize,
    lo: usize,
    hi: usize,
    rhs_dests: Vec<usize>,
    neighbors: &mut as_waksman_topology,
) {
    if left > right {
        return;
    }
    ...
    if right - left + 1 > subnetwork_width {
        ...
        for packet_idx in lo..=hi {
            (neighbors[left][packet_idx].0, neighbors[left][packet_idx].1) =
                (packet_idx, packet_idx);
            (
                neighbors[right][packet_idx].0,
                neighbors[right][packet_idx].1,
            ) = (rhs_dests[packet_idx - lo], rhs_dests[packet_idx - lo]);
        }

        let mut new_rhs_dests = vec![usize::MIN / 2; subnetwork_size];
        for packet_idx in lo..=hi {
        ...
        for row_idx in (lo..hi + 1 - subnetwork_size % 2).step_by(2) {
            neighbors[left][row_idx].0 =
                as_waksman_switch_output(subnetwork_size, lo, row_idx, true);
            neighbors[left][row_idx + 1].1 = neighbors[left][row_idx].0;
            neighbors[left][row_idx].1 =
                as_waksman_switch_output(subnetwork_size, lo, row_idx, false);
            neighbors[left][row_idx + 1].0 = neighbors[left][row_idx].1;

            new_rhs_dests[as_waksman_switch_input(subnetwork_size, lo, row_idx, true) - lo] =
                row_idx;
            new_rhs_dests[as_waksman_switch_input(subnetwork_size, lo, row_idx, false) - lo] =
                row_idx + 1;

            neighbors[right][row_idx].0 = rhs_dests[row_idx - lo];
            neighbors[right][row_idx + 1].1 = rhs_dests[row_idx - lo];
            neighbors[right][row_idx].1 = rhs_dests[row_idx + 1 - lo];
            neighbors[right][row_idx + 1].0 = rhs_dests[row_idx + 1 - lo];
        }
        ...
        if subnetwork_size % 2 == 1 {
            ...
            (neighbors[left][hi].0, neighbors[left][hi].1) = (hi, hi);
            (neighbors[right][hi].0, neighbors[right][hi].1) =
                (rhs_dests[hi - lo], rhs_dests[hi - lo]);
            ...
            neighbors[left][hi - 1].1 = neighbors[left][hi - 1].0;
            neighbors[left][hi].1 = neighbors[left][hi].0;
        }

        let d = as_waksman_top_height(subnetwork_size);
        let new_rhs_dests_top = &new_rhs_dests[..d];
...
pub fn generate_as_waksman_topology(num_packets: usize) -> as_waksman_topology {
    ...
    let mut neighbors = vec![vec![(usize::MIN / 2, usize::MIN / 2); num_packets]; width];
    ...
    construct_as_waksman_inner(0, width - 1, 0, num_packets - 1, rhs_dests, &mut neighbors);
 ...
 */
pub fn as_waksman_get_switch_setting_from_top_bottom_decision(
    row_offset: usize,
    packet_idx: usize,
    use_top: bool,
) -> bool {
 ...
 */
pub fn as_waksman_get_top_bottom_decision_from_switch_setting(
    row_offset: usize,
    packet_idx: usize,
    switch_setting: bool,
) -> bool {
...
pub fn as_waksman_route_inner(
    ...
    routing: &mut as_waksman_routing,
) {
    if left > right {
        return;
    }
    ...
    if right - left + 1 > subnetwork_width {
        ...
        if subnetwork_size % 2 == 1 {
            ...
            if permutation.get(hi) == hi {
                ...
                let rhs_switch_setting = as_waksman_get_switch_setting_from_top_bottom_decision(
                    lo,
                    permutation.get(hi),
                    false,
                );
        ...
        loop {
            ...
            if route_left {
                ...
                let use_top = as_waksman_get_top_bottom_decision_from_switch_setting(
                    lo,
                    to_route,
                    lhs_switch_setting,
                );
                ...
                if permutation.get(to_route) == hi {
                     ...
                     */
                    assert!(routing[right].get(rhs_switch).is_none());
                ...
                assert!(routing[right].get(rhs_switch).is_some());
                ...
                let lhs_switch_setting = as_waksman_get_switch_setting_from_top_bottom_decision(
                    lo,
                    permutation_inv.get(to_route),
                    use_top,
                );
...
pub fn valid_as_waksman_routing(
    permutation: &integer_permutation,
    ...
    for column_idx in 0..width {
        ...
        for packet_idx in 0..num_packets {
            ...
            if neighbors[column_idx][packet_idx].0 == neighbors[column_idx][packet_idx].1 {
                routed_packet_idx = neighbors[column_idx][packet_idx].0;
            } else {
                let it = routing[column_idx].get(packet_idx);
                let it2 = routing[column_idx].get(packet_idx - 1);
                assert!(it.is_some() ^ it2.is_some());
                let switch_setting = if let Some(_v) = routing.get(column_idx) {
                    it.unwrap()
                } else {
                    it2.unwrap()
                };

                routed_packet_idx = if *switch_setting {
                    neighbors[column_idx][packet_idx].1
                } else {
                    neighbors[column_idx][packet_idx].0
                };
            }
