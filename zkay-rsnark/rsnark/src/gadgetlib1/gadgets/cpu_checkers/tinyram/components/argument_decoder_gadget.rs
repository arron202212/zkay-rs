...
impl<FieldT: FieldTConfig> argument_decoder_gadget<FieldT> {
    pub fn new(
        ...
        let pack_desidx = RcCell::new(tinyram_packing_gadget::<FieldT>::new(
            pb.clone(),
            desidx.clone().into(),
            packed_desidx.clone().into(),
            format!("{}pack_desidx", annotation_prefix),
        ));
        let pack_arg1idx = RcCell::new(tinyram_packing_gadget::<FieldT>::new(
            pb.clone(),
            arg1idx.clone().into(),
            packed_arg1idx.clone().into(),
            format!("{}pack_arg1idx", annotation_prefix),
        ));
        let pack_arg2idx = RcCell::new(tinyram_packing_gadget::<FieldT>::new(
            pb.clone(),
            arg2idx.clone().into(),
        ...
        let demux_des = RcCell::new(tinyram_loose_multiplexing_gadget::<FieldT>::new(
            pb.clone(),
        ...
        let demux_arg1 = RcCell::new(tinyram_loose_multiplexing_gadget::<FieldT>::new(
            pb.clone(),
            packed_registers.clone().into(),
            packed_arg1idx.clone().into(),
        ...
        let demux_arg2 = RcCell::new(tinyram_loose_multiplexing_gadget::<FieldT>::new(
            pb.clone(),
            packed_registers.clone().into(),
            packed_arg2idx.clone().into(),
    ...
    pub fn generate_r1cs_constraints(&self) {
        /* pack */
        self.t.t.t.pack_desidx.generate_r1cs_constraints(true);
        self.t.t.t.pack_arg1idx.generate_r1cs_constraints(true);
        self.t.t.t.pack_arg2idx.generate_r1cs_constraints(true);

        /* demux */
        self.t.t.t.demux_des.generate_r1cs_constraints();
        self.t.t.t.demux_arg1.generate_r1cs_constraints();
        self.t.t.t.demux_arg2.generate_r1cs_constraints();
        ...
        self.pb.borrow_mut().add_r1cs_constraint(
            r1cs_constraint::<FieldT>::new_with_vec(
                vec![ONE, self.t.t.t.arg2_is_imm.clone() * (-1)],
                vec![ONE, self.t.t.t.arg2_demux_success.clone() * (-1)],
                vec![ONE * 0],
            ),
        ...
        */
        self.pb.borrow_mut().add_r1cs_constraint(
            r1cs_constraint::<FieldT>::new_with_vec(
                vec![self.t.t.t.arg2_is_imm.clone()],
                vec![
                    self.t.t.t.packed_arg2idx.clone(),
                    self.t.t.t.arg2_demux_result.clone() * (-1),
                ],
                vec![
                    self.t.t.t.packed_arg2val.clone(),
                    self.t.t.t.arg2_demux_result.clone() * (-1),
                ],
            ),
    ...
    pub fn generate_r1cs_witness(&self) {
        /* pack */
        self.t.t.t.pack_desidx.generate_r1cs_witness_from_bits();
        self.t.t.t.pack_arg1idx.generate_r1cs_witness_from_bits();
        self.t.t.t.pack_arg2idx.generate_r1cs_witness_from_bits();

        /* demux */
        self.t.t.t.demux_des.generate_r1cs_witness();
        self.t.t.t.demux_arg1.generate_r1cs_witness();
        self.t.t.t.demux_arg2.generate_r1cs_witness();
...
pub fn test_argument_decoder_gadget() {
    ...
    let mut pb = RcCell::new(tinyram_protoboard::<FieldT>::new(ap, P.len(), 0, 10));
    ...
    for i in 0..ap.k {
        pb.borrow().val(&packed_registers[i]) = FieldT(1000 + i);
    }

    pb.borrow().val(&desidx.packed) = FieldT(2);
    pb.borrow().val(&arg1idx.packed) = FieldT(5);
    pb.borrow().val(&arg2idx.packed) = FieldT(7);
    pb.borrow().val(&arg2_is_imm) = FieldT::zero();
    ...
    assert!(pb.borrow().val(&packed_desval) == FieldT(1002));
    assert!(pb.borrow().val(&packed_arg1val) == FieldT(1005));
    assert!(pb.borrow().val(&packed_arg2val) == FieldT(1007));
    assert!(pb.is_satisfied());
    print!("positive test (get reg) successful\n");

    pb.borrow().val(&arg2_is_imm) = FieldT::one();
    g.generate_r1cs_witness();

    assert!(pb.borrow().val(&packed_desval) == FieldT(1002));
    assert!(pb.borrow().val(&packed_arg1val) == FieldT(1005));
    assert!(pb.borrow().val(&packed_arg2val) == FieldT(7));
    assert!(pb.is_satisfied());
