...
// 
// pb_linear_combination_array<FieldT> SHA256_default_IV(pb:protoboard<FieldT> );
...
;
 ...
 pub fn SHA256_default_IV(pb:protoboard<FieldT> )->pb_linear_combination_array<FieldT>
{
    ...
    
{
    ...
    for i in 0..16
    {
        ...
        pack_W[i].reset(packing_gadget::<FieldT>::new(pb, W_bits[i], packed_W[i], FMT(self.annotation_prefix, " pack_W_{}", i)));
    }
    ...
    for i in 16..64
    {
        ...
        compute_sigma0[i].reset(small_sigma_gadget::<FieldT>::new(pb, W_bits[i-15], sigma0[i], 7, 18, 3, FMT(self.annotation_prefix, " compute_sigma0_{}", i)));
        compute_sigma1[i].reset(small_sigma_gadget::<FieldT>::new(pb, W_bits[i-2], sigma1[i], 17, 19, 10, FMT(self.annotation_prefix, " compute_sigma1_{}", i)));
        ...
        mod_reduce_W[i].reset(lastbits_gadget::<FieldT>::new(pb, unreduced_W[i], 32+2, packed_W[i], W_bits[i], FMT(self.annotation_prefix, " mod_reduce_W_{}", i)));
    }
...
pub fn generate_r1cs_witness()
{
    ...
    for i in 16..64
    {
        ...
        self.pb.val(unreduced_W[i]) = self.pb.val(sigma0[i]) + self.pb.val(sigma1[i]) + self.pb.val(packed_W[i-16]) + self.pb.val(packed_W[i-7]);
   ...
   
{
    ...
    compute_sigma0.reset(big_sigma_gadget::<FieldT>::new(pb, a, sigma0, 2, 13, 22, FMT(self.annotation_prefix, " compute_sigma0")));
    compute_sigma1.reset(big_sigma_gadget::<FieldT>::new(pb, e, sigma1, 6, 11, 25, FMT(self.annotation_prefix, " compute_sigma1")));

    /* compute choice */
    choice.allocate(pb, FMT(self.annotation_prefix, " choice"));
    compute_choice.reset(choice_gadget::<FieldT>::new(pb, e, f, g, choice, FMT(self.annotation_prefix, " compute_choice")));

    /* compute majority */
    majority.allocate(pb, FMT(self.annotation_prefix, " majority"));
    compute_majority.reset(majority_gadget::<FieldT>::new(pb, a, b, c, majority, FMT(self.annotation_prefix, " compute_majority")));

    /* pack d */
    packed_d.allocate(pb, FMT(self.annotation_prefix, " packed_d"));
    pack_d.reset(packing_gadget::<FieldT>::new(pb, d, packed_d, FMT(self.annotation_prefix, " pack_d")));

    /* pack h */
    packed_h.allocate(pb, FMT(self.annotation_prefix, " packed_h"));
    pack_h.reset(packing_gadget::<FieldT>::new(pb, h, packed_h, FMT(self.annotation_prefix, " pack_h")));
    ...
    mod_reduce_new_a.reset(lastbits_gadget::<FieldT>::new(pb, unreduced_new_a, 32+3, packed_new_a, new_a, FMT(self.annotation_prefix, " mod_reduce_new_a")));
    mod_reduce_new_e.reset(lastbits_gadget::<FieldT>::new(pb, unreduced_new_e, 32+3, packed_new_e, new_e, FMT(self.annotation_prefix, " mod_reduce_new_e")));
    //  gadget<FieldT>(pb, annotation_prefix),
    Self{a,
    b,
    c,
...
pub fn generate_r1cs_witness()
{
    ...
    self.pb.val(unreduced_new_a) = self.pb.val(packed_h) + self.pb.val(sigma1) + self.pb.val(choice) + FieldT(K) + self.pb.val(W) + self.pb.val(sigma0) + self.pb.val(majority);
    self.pb.val(unreduced_new_e) = self.pb.val(packed_d) + self.pb.val(packed_h) + self.pb.val(sigma1) + self.pb.val(choice) + FieldT(K) + self.pb.val(W);
