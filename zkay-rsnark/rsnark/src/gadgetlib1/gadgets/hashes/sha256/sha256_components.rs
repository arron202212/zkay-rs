    ...
    packed_W(packed_W)
{
    ...
    for (size_t i = 16; i < 64; ++i)
    {
        ...
        compute_sigma0[i].reset(new small_sigma_gadget<FieldT>(pb, W_bits[i-15], sigma0[i], 7, 18, 3, FMT(self.annotation_prefix, " compute_sigma0_{}", i)));
        compute_sigma1[i].reset(new small_sigma_gadget<FieldT>(pb, W_bits[i-2], sigma1[i], 17, 19, 10, FMT(self.annotation_prefix, " compute_sigma1_{}", i)));
        ...
        mod_reduce_W[i].reset(new lastbits_gadget<FieldT>(pb, unreduced_W[i], 32+2, packed_W[i], W_bits[i], FMT(self.annotation_prefix, " mod_reduce_W_{}", i)));
    }
}
    ...
    new_e(new_e)
{
    ...
    compute_sigma0.reset(new big_sigma_gadget<FieldT>(pb, a, sigma0, 2, 13, 22, FMT(self.annotation_prefix, " compute_sigma0")));
    compute_sigma1.reset(new big_sigma_gadget<FieldT>(pb, e, sigma1, 6, 11, 25, FMT(self.annotation_prefix, " compute_sigma1")));

    /* compute choice */
    choice.allocate(pb, FMT(self.annotation_prefix, " choice"));
    compute_choice.reset(new choice_gadget<FieldT>(pb, e, f, g, choice, FMT(self.annotation_prefix, " compute_choice")));

    /* compute majority */
    majority.allocate(pb, FMT(self.annotation_prefix, " majority"));
    compute_majority.reset(new majority_gadget<FieldT>(pb, a, b, c, majority, FMT(self.annotation_prefix, " compute_majority")));

    /* pack d */
    packed_d.allocate(pb, FMT(self.annotation_prefix, " packed_d"));
    pack_d.reset(new packing_gadget<FieldT>(pb, d, packed_d, FMT(self.annotation_prefix, " pack_d")));

    /* pack h */
    packed_h.allocate(pb, FMT(self.annotation_prefix, " packed_h"));
    pack_h.reset(new packing_gadget<FieldT>(pb, h, packed_h, FMT(self.annotation_prefix, " pack_h")));
    ...
    mod_reduce_new_a.reset(new lastbits_gadget<FieldT>(pb, unreduced_new_a, 32+3, packed_new_a, new_a, FMT(self.annotation_prefix, " mod_reduce_new_a")));
    mod_reduce_new_e.reset(new lastbits_gadget<FieldT>(pb, unreduced_new_e, 32+3, packed_new_e, new_e, FMT(self.annotation_prefix, " mod_reduce_new_e")));
}
