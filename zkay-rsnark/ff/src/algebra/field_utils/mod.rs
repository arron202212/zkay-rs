...
macro_rules! BigInt {
    ($c0:expr) => {{
        let (is_positive, limbs) = $crate::algebra::fields::ff_macros::to_sign_and_limbs!($c0);
        assert!(is_positive);
        let mut integer = $crate::algebra::field_utils::BigInt::zero();
...
impl<const N: usize> BigInt<N> {
    ...
    pub const fn montgomery_r(&self) -> Self {
        let two_pow_n_times_64 = crate::algebra::const_helpers::RBuffer([0u64; N], 1);
...
impl<const N: usize> BigInteger for BigInt<N> {
    ...
    fn mul(&self, other: &Self) -> (Self, Self) {
        ...
        let mut r = crate::algebra::const_helpers::MulBuffer::zeroed();
