use crate::FpmConfig;
use crate::algebra::curves::edwards::edwards_fields::{edwards_Fq, edwards_Fr};
use crate::algebra::curves::edwards::edwards_init::{
    edwards_coeff_d, edwards_twist_mul_by_a_c0, edwards_twist_mul_by_d_c1,
...
impl edwards_G1 {
    pub fn new(X: edwards_Fq, Y: edwards_Fq, Z: edwards_Fq) -> Self {
        Self { X, Y, Z }
    }

    // using inverted coordinates
    pub fn new2(X: edwards_Fq, Y: edwards_Fq) -> Self {
        let Z = X * Y;
        Self { X, Y, Z }
    }

    pub fn size_in_bits() -> usize {
        return edwards_Fq::ceil_size_in_bits() + 1;
    }
...
impl edwards_G1 {
    ...
    pub fn add(&self, other: &edwards_G1) -> edwards_G1 {
    ...
    pub fn mixed_add(&self, other: &edwards_G1) -> edwards_G1 {
        // #ifdef PROFILE_OP_COUNTS
        // self.add_cnt += 1;
...
impl Add<i32> for edwards_G1 {
    type Output = edwards_G1;
...
impl<O: Borrow<Self>> Add<O> for edwards_G1 {
    type Output = edwards_G1;
...
impl<const N: usize> Mul<bigint<N>> for edwards_G1 {
    type Output = edwards_G1;
...
impl<const N: usize, T: Fp_modelConfig<N>> Mul<Fp_model<N, T>> for edwards_G1 {
    type Output = edwards_G1;
...
impl<O: Borrow<Self>> Mul<O> for edwards_G1 {
    type Output = edwards_G1;
...
use std::fmt;
impl fmt::Display for edwards_G1 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", Self::one())
    }
}
impl One for edwards_G1 {
    fn one() -> Self {
        Self::one()
    }
}

impl Zero for edwards_G1 {
    fn zero() -> Self {
        Self::zero()
    }
...
// impl<'a> Mul<&'a EdwardsG1> for &'a BigInt {
//     type Output = EdwardsG1;
//     fn mul(self, rhs: &'a EdwardsG1) -> EdwardsG1 {
//         rhs.scalar_mul(self)
//     }
// }

// impl PartialEq for EdwardsG1 {
...
// impl<'a> Add<&'a EdwardsG1> for &'a EdwardsG1 {
//     type Output = EdwardsG1;
//     fn add(self, other: &'a EdwardsG1) -> EdwardsG1 {
//         if self.is_zero() {
//             return *other;
//         }
//         if other.is_zero() {
...
// impl<'a> Sub<&'a EdwardsG1> for &'a EdwardsG1 {
//     type Output = EdwardsG1;
//     fn sub(self, other: &'a EdwardsG1) -> EdwardsG1 {
//         self + &(-*other)
//     }
// }
...
//             let x2 = t_x.square();
//             let num = Fq::one() - &x2;
//             let den = (Fq::one() - &(EDWARDS_D * &x2)).inverse().unwrap();
//             let mut t_y = (num * &den)
//                 .sqrt()
//                 .ok_or(io::Error::new(io::ErrorKind::InvalidData, "Invalid point"))?;
