...
impl<FieldT: FieldTConfig> EvaluationDomainConfig<FieldT> for geometric_sequence_domains<FieldT> {
    fn FFT(&mut self, a: &mut Vec<FieldT>) -> eyre::Result<()> {
        ...
        monomial_to_newton_basis_geometric(
            a,
            &self.t.geometric_sequence,
            &self.t.geometric_triangular_sequence,
            self.m,
        );
        ...
        for i in 1..self.m {
            // T[i] = T[i-1] * (self.t.geometric_sequence[i] - FieldT::one()).inverse();
            g[i] = self.t.geometric_triangular_sequence[i].clone() * a[i].clone();
        }
    ...
    fn iFFT(&mut self, a: &mut Vec<FieldT>) -> eyre::Result<()> {
        ...
        for i in 1..self.m {
            // prev_T *= (self.t.geometric_sequence[i].clone() - FieldT::one()).inverse();

            // W[i] = a[i].clone() * prev_T.clone();
            T[i] = self.t.geometric_triangular_sequence[i].clone() * prev_T.clone();
            if i % 2 == 1 {
                T[i] = -T[i].clone();
            }
        }
    ...
    fn evaluate_all_lagrange_polynomials(&mut self, t: &FieldT) -> Vec<FieldT> {
        ...
        for i in 1..self.m {
            l[i] = tt.clone() - self.t.geometric_sequence[i].clone();
            g[i] = FieldT::one() - self.t.geometric_sequence[i].clone();

            l_vanish *= l[i].clone();
            g_vanish *= g[i].clone();
        }

        //   let mut  r = self.t.geometric_sequence[self.m-1].inverse();
        //   let mut  r_i = r.clone();
        ...
        for i in 1..self.m {
            // g_i[i] = g_i[i-1].clone() * g[self.m-i].clone() * -g[i].clone().inverse() * self.t.geometric_sequence[i].clone();
    ...
    fn get_domain_element(&mut self, idx: usize) -> FieldT {
        ...
        return self.t.geometric_sequence[idx].clone();
    }
    ...
    fn compute_vanishing_polynomial(&mut self, t: &FieldT) -> FieldT {
        ...
        let tt: FieldT = t.clone();
        for i in 0..self.m {
            Z *= (tt.clone() - self.t.geometric_sequence[i].clone());
        }
        return Z;
    }
    ...
    fn add_poly_Z(&mut self, coeff: &FieldT, H: &mut Vec<FieldT>) -> eyre::Result<()> {
        ...
        let mut x = vec![-self.t.geometric_sequence[0].clone(), FieldT::one()];

        let mut t = vec![FieldT::zero(); 2];

        for i in 1..self.m + 1 {
            t[0] = -self.t.geometric_sequence[i].clone();
            t[1] = FieldT::one();
            let xx = x.clone();
...
impl<FieldT: FieldTConfig> geometric_sequence_domains<FieldT> {
    fn do_precomputation(&mut self) {
        self.t.geometric_sequence = vec![FieldT::zero(); self.m];
        self.t.geometric_sequence[0] = FieldT::one();

        self.t.geometric_triangular_sequence = vec![FieldT::zero(); self.m];
        self.t.geometric_triangular_sequence[0] = FieldT::one();

        for i in 1..self.m {
            self.t.geometric_sequence[i] = self.t.geometric_sequence[i - 1].clone() * FieldT::one(); //geometric_generator();
            self.t.geometric_triangular_sequence[i] = self.t.geometric_triangular_sequence[i - 1]
                .clone()
                * self.t.geometric_sequence[i - 1].clone();
        }
